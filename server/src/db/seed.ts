...
async function seed() {
  ...
  const q2Initiatives = [
    ...
    { id: "creator",      team: "RDB-KV", name: "Creator DataStore: source of truth for 26 universes", theme: "Cust Exp",   pri: "P0", deliverables: "RDB as prod source of truth for 26 universes. Auto-fallback DDB/S3.", metrics: "100% consistency vs DDB/S3; p99 ≤ baseline; PRR approved" },
    { id: "frost",        team: "RDB-KV", name: "Frost Personalization: 100% traffic on RDB",         theme: "Cust Exp",   pri: "P0", deliverables: "Frost traffic 50%→100% on RDB source of truth.", metrics: "100% Frost reads on RDB; p99 ≤ DDB baseline" },
    { id: "cell",         team: "RDB-KV", name: "Cell stability: continuous patching + HA",           theme: "Reliability", pri: "P0", deliverables: "Continuous patching with <0.1% error rate. Single-cell HA validated.", metrics: "Patching <0.1% error; HA drill passing; 0 P0 correctness" },
    { id: "raft",         team: "RDB-KV", name: "Platform foundation: Raft consensus + leader failover", theme: "Reliability", pri: "P0", deliverables: "Raft consensus engine. Leader failover validated. 150 MB/s perf baseline.", metrics: "Leader failover <5s; 150 MB/s sustained; 0 P0 correctness" },
    { id: "multitenancy", team: "RDB-KV", name: "Platform foundation: multi-tenancy dedicated placement", theme: "Reliability", pri: "P0", deliverables: "Dedicated placement for 26+ universes. Tenant isolation enforced.", metrics: "All 26 universes on dedicated placement; 0 cross-tenant incidents" },
    { id: "bandrestore",  team: "RDB-KV", name: "Platform foundation: backup & restore production drill", theme: "Reliability", pri: "P0", deliverables: "Production backup pipeline. Full restore drill executed.", metrics: "Restore drill passing; RPO ≤1h; 0 data loss" },
    ...
    { id: "oncalltoil", team: "EaaS",  name: "Oncall Toil reduction",                     theme: "Reliability", pri: "P1" },
  ...
  const q2Backlog = [
    ...
    // ── Ready ──
    { id: "b-disc-cqd",  team: "RDB-KV", name: "Discovery: content quality detection storage",          theme: "Cust Exp",   readiness: "ready",  problemValue: "KV store needed for clickbait/scam detection signals; requires wide-column partial update support.", successMetric: "Wide-column KV live for Discovery use case.", effort: "M",   earliest: "Q2 2026",  requestorDri: "Discovery Team",     nextAction: "Scope partial update support" },
    { id: "b-kafka-px",  team: "QaaS",   name: "Central Kafka proxy (quota enforcement + 40T msg/day)", theme: "Cust Exp",   readiness: "ready",  problemValue: "Scale 12.7T→40T msg/day for 60M+ PCP; quota enforcement missing; 1.5 HC partially funded.", successMetric: "Proxy live; quota on 100% ingestion; 40T/day capacity.", effort: "L",   earliest: "Q3 2026",  requestorDri: "Jeffrey Zhong",      nextAction: "Confirm remaining HC" },
    { id: "b-cost-dash", team: "All",    name: "Cost-to-serve dashboards v1",                           theme: "Efficiency",  readiness: "ready",  problemValue: "Per-service cost planning is manual. No Grafana visibility.", successMetric: "Per-service cost visible in Grafana; capacity model updated.", effort: "M",   earliest: "H2 2026",  requestorDri: "Finance / Storage Eng", nextAction: "Define metrics schema" },
    { id: "b-raas-gate", team: "RaaS",   name: "Automatic gating for downsizing and deletion",          theme: "Reliability", readiness: "ready",  problemValue: "Accidental RaaS cluster downsizing/deletion causes outages. No guardrails.", successMetric: "Zero accidental deletions; gating enforced.", effort: "M",   earliest: "H2 2026",  requestorDri: "Sen Li",             nextAction: "Design guardrail policy" },
    { id: "b-raas-hol",  team: "RaaS",   name: "Head-of-line blocking mitigation",                      theme: "Reliability", readiness: "ready",  problemValue: "Read/write pipeline contention on envoy proxy degrades latency under load.", successMetric: "p99 read latency stable under write bursts.", effort: "L",   earliest: "H2 2026",  requestorDri: "Sen Li",             nextAction: "Prototype separate pipelines" },
    { id: "b-raas-cli",  team: "RaaS",   name: "Client standardization (full feature parity)",          theme: "Cust Exp",   readiness: "ready",  problemValue: "Feature gaps across BEDEV1/BEDEV2/Golang/Rust clients. Customers hit unexpected limits.", successMetric: "100% feature parity across all clients.", effort: "XL",  earliest: "H2 2026",  requestorDri: "Sen Li",             nextAction: "Gap analysis across clients" },
    { id: "b-raas-rel",  team: "RaaS",   name: "Release flow improvement (staged rollout + health checks)", theme: "Reliability", readiness: "ready", problemValue: "No staged rollout or automated health checks in CICD. Risky deploys.", successMetric: "100% deploys via staged rollout; zero deploy-caused outages.", effort: "M",   earliest: "H2 2026",  requestorDri: "Sen Li",             nextAction: "Integrate with CICD" },
    { id: "b-raas-mtc",  team: "RaaS",   name: "Multi-tenant cache",                                    theme: "Efficiency",  readiness: "ready",  problemValue: "Shared cache cluster needed for resource/access/failure isolation. ~20% fleet savings projected.", successMetric: "Multi-tenant cache live; 20% fleet reduction.", effort: "XL",  earliest: "H2 2026",  requestorDri: "Sen Li",             nextAction: "Architecture design" },
    { id: "b-selfserve", team: "All",    name: "Self-service storage provisioning",                      theme: "Efficiency",  readiness: "ready",  problemValue: "Manual provisioning tickets slow dev velocity. Waiting on RDB/CRDB/R3 APIs to stabilize.", successMetric: "Self-service portal live; ticket volume -50%.", effort: "L",   earliest: "H2 2026",  requestorDri: "Platform Eng",       nextAction: "Confirm API readiness" },
    { id: "b-antithesis",team: "RDB-KV", name: "Antithesis POC + perf validation",                      theme: "Reliability", readiness: "ready",  problemValue: "No fault injection / chaos testing. Reliability regressions caught late.", successMetric: "Antithesis POC validated; 1 fault scenario automated.", effort: "M",   earliest: "Q3 2026",  requestorDri: "RDB-KV Eng",         nextAction: "Run POC in staging" },
    { id: "b-pitr",      team: "RDB-KV", name: "PITR (point-in-time recovery)",                         theme: "Reliability", readiness: "ready",  problemValue: "No PITR for RDB-KV. Data loss risk on corruption events. Depends on CDC engine.", successMetric: "PITR RPO ≤1h validated in staging.", effort: "M",   earliest: "Q4 2026",  requestorDri: "RDB-KV Eng",         nextAction: "Gate on CDC prototype" },
    { id: "b-crdb2pg",   team: "RDB-PG", name: "CRDB-to-PG data movement tooling",                      theme: "Cust Exp",   readiness: "ready",  problemValue: "No tooling to migrate CRDB workloads to Postgres. Manual migration is error-prone.", successMetric: "≥1 CRDB workload migrated to PG; tooling reusable.", effort: "L",   earliest: "Q3 2026",  requestorDri: "Zhengyin Qian",      nextAction: "Scope migration tooling" },
    { id: "b-r3-assets", team: "R3",     name: "Feature support: asset migration (versioning + lifecycle)", theme: "Cust Exp", readiness: "ready",  problemValue: "Object versioning + lifecycle features missing. Asset pipeline migration blocked. HC moved to DR/Security.", successMetric: "Versioning + lifecycle live; ≥1 asset use case migrated.", effort: "M",   earliest: "Q3 2026",  requestorDri: "Derek Pham",         nextAction: "Reallocate HC after DR+EC complete" },
    // ── Needs scoping ──
    { id: "b-mosaic",    team: "EaaS",   name: "EaaS ↔ Mosaic (schema mgmt + health dashboard)",        theme: "Cust Exp",   readiness: "scope",  problemValue: "No schema management or health dashboard for EaaS customers. No confirmed DRI.", successMetric: "Schema registry live; health dashboard adopted by 3+ teams.", effort: "M",   earliest: "Q3 2026",  requestorDri: "Gavin Wang",         nextAction: "Confirm DRI; reliability-first" },
    { id: "b-configmgmt",team: "EaaS",   name: "Config management: breakglass + config-as-code",        theme: "Reliability", readiness: "scope",  problemValue: "No HC for joint EE config-as-code. Drift detection and breakglass missing.", successMetric: "Config-as-code enforced; breakglass procedure documented.", effort: "S",   earliest: "TBD",      requestorDri: "Kai Liu",            nextAction: "Scope with EE team" },
    { id: "b-bloom",     team: "EaaS",   name: "Bloom filters in EaaS",                                 theme: "Cust Exp",   readiness: "scope",  problemValue: "40M+ evals/sec across Safety/Economy/Ads need Bloom filter service. Current owner unresourced.", successMetric: "YAML-configured Bloom filters; 15+ use cases migrated.", effort: "XL",  earliest: "2027",     requestorDri: "Eitan Rothberg",     nextAction: "Revisit Q4 2026 planning" },
    { id: "b-eaas2pg",   team: "RDB-PG", name: "EaaS-to-PG integration + data movement tooling",        theme: "Cust Exp",   readiness: "scope",  problemValue: "CRDB/MSSQL migration blocked. Gated on +3 PG engineers (Option A). Leadership decision required.", successMetric: "3-5 use cases migrated to PG single-region.", effort: "XL",  earliest: "Q3 2026",  requestorDri: "Yingjie He",         nextAction: "Leadership HC decision by EOQ2" },
    { id: "b-cdc",       team: "RDB-KV", name: "CDC engine: ordered change stream per shard",            theme: "Reliability", readiness: "scope",  problemValue: "No change stream. PITR, OrderedDataStore, and tenant moves all blocked. Needs dedicated HC.", successMetric: "CDC prototype live; ordered stream + replay + checkpoint.", effort: "L",   earliest: "Q3 2026",  requestorDri: "Dikang Gu",          nextAction: "Allocate dedicated HC" },
    { id: "b-tenant-mv", team: "RDB-KV", name: "Tenant move pipeline",                                  theme: "Cust Exp",   readiness: "scope",  problemValue: "Manual mapping for 26 universes. 5M universe automation needs CDC prototype first.", successMetric: "Automated tenant move for 100% universes.", effort: "L",   earliest: "Q4 2026",  requestorDri: "Zhixin Wen",         nextAction: "Gate on CDC prototype" },
    { id: "b-e2e-infra", team: "EaaS",   name: "Integration test infra: E2E suite + fault injection",   theme: "Reliability", readiness: "scope",  problemValue: "Q2 ships unit tests + canary only. Full E2E + fault injection deferred to Q3.", successMetric: "E2E suite covering 5+ critical paths; fault injection automated.", effort: "M",   earliest: "Q3 2026",  requestorDri: "EaaS Eng",           nextAction: "Scope Q3 test infra" },
    { id: "b-ev1-long",  team: "EaaS",   name: "EV1 long tail: 95% local cache removal (160 EGs)",      theme: "Cust Exp",   readiness: "scope",  problemValue: "Only 25% assets migration in Q2. Full long-tail (160 entity groups) deferred.", successMetric: "95% EV1 local cache removed; 160 entity groups migrated.", effort: "L",   earliest: "Q4 2026",  requestorDri: "EaaS Eng",           nextAction: "Sequence after Q3 assets migration" },
    { id: "b-r3-cap",    team: "R3",     name: "R3 capacity expansion: new use-case onboarding",        theme: "Reliability", readiness: "scope",  problemValue: "R3 capacity constrains new use-case onboarding + region failover. On-prem expansion needed.", successMetric: "R3 capacity expanded; ≥2 new use cases onboarded.", effort: "L",   earliest: "Q2 2026",  requestorDri: "Danny Yuan",         nextAction: "Budget ask TBD" },
    { id: "b-pg-found",  team: "RDB-PG", name: "PG engine foundation: B&R + PITR + TLS + cell HA",      theme: "Reliability", readiness: "scope",  problemValue: "Team restructured (4 SWEs moved to KV). Gated on Option A HC decision. Core PG reliability blockers.", successMetric: "B&R validated; PITR RPO ≤1h; TLS live; Cell HA drill passing.", effort: "XL",  earliest: "Q3 2026",  requestorDri: "Yingjie He",         nextAction: "HC decision by EOQ2" },
    { id: "b-pg-sql",    team: "RDB-PG", name: "Direct-SQL use cases (LLM proxy, TestHub, Airflow, Vector)", theme: "Cust Exp", readiness: "scope", problemValue: "4 use cases descoped when team restructured. High customer demand.", successMetric: "≥2 of 4 use cases live on PG.", effort: "L",   earliest: "Q3 2026",  requestorDri: "Yingjie He",         nextAction: "Revisit Q3 after KV milestones" },
    { id: "b-pg-sot",    team: "RDB-PG", name: "PG: 4 use-cases to SoT + multi-region HA",              theme: "Reliability", readiness: "scope",  problemValue: "Vector Search + others need SoT promotion. CHI/ASH multi-region needed. Gated on Option A.", successMetric: "≥4 use cases at SoT; multi-region HA in CHI+ASH.", effort: "L",   earliest: "Q3 2026",  requestorDri: "Yingjie He",         nextAction: "Gate on Option A HC" },
    // ── Parked ──
    { id: "b-starrocks", team: "SIM",    name: "StarRocks + R3 logging analytics POC",                   theme: "Efficiency",  readiness: "parked", problemValue: "ClickHouse consolidation deferred. Need cost/perf analysis vs existing ClickHouse Cloud.", successMetric: "≥40% cost saving vs ClickHouse Cloud validated.", effort: "XL",  earliest: "Q3 2026",  requestorDri: "Danny Yuan",         nextAction: "Run cost/perf analysis" },
    { id: "b-ods",       team: "RDB-KV", name: "OrderedDataStore on RDB",                                theme: "Cust Exp",   readiness: "parked", problemValue: "Requires CDC engine + Materializer + list-keys + ODS indexes. All dependencies unbuilt.", successMetric: "100% OrderedDataStore on RDB.", effort: "XL",  earliest: "H1 2027",  requestorDri: "RDB-KV Eng",         nextAction: "Revisit H2 2026 planning" },
    { id: "b-slo-enf",   team: "EaaS",   name: "SLO enforcement & capacity dashboards",                  theme: "Reliability", readiness: "parked", problemValue: "SLO scorecard + self-service dashboards deferred. Only FC metrics shipped in Q2.", successMetric: "SLO enforcement live; self-service dashboards for 5+ teams.", effort: "M",   earliest: "Q3 2026",  requestorDri: "EaaS Eng",           nextAction: "Scope 3 unfunded items for Q3" },
    { id: "b-autoscale", team: "RDB-KV", name: "Adaptive auto-scaler (spike detection)",                 theme: "Reliability", readiness: "parked", problemValue: "Spike detection + capacity management frameworks not yet built. Revisit after those ship.", successMetric: "Auto-scaler handling 2× traffic spikes without manual intervention.", effort: "L",   earliest: "H1 2027",  requestorDri: "Storage Eng",        nextAction: "Gate on capacity mgmt framework" },
    { id: "b-mssql-hw",  team: "MS SQL", name: "MS SQL hardware refresh phase 1 (55 servers, CHI)",      theme: "Reliability", readiness: "parked", problemValue: "Hardware arrives September earliest. Budget approved, PO in progress. 55 servers, ~$5M, CHI.", successMetric: "55 servers racked and operational; old hardware decommissioned.", effort: "XL",  earliest: "Q4 2026",  requestorDri: "Gopal Anand",        nextAction: "Await hardware delivery" },
    // ── Watching / deferred ──
    { id: "b-crdb-cell", team: "SIM",    name: "CRDB on Roblox Cell Engine",                             theme: "Reliability", readiness: "watch",  problemValue: "Zero-downtime workload shift from pre-cell to Nomad. Waiting on Compute team scoping.", successMetric: "CRDB workloads running on Cell Engine with zero downtime migration.", effort: "TBD", earliest: "TBD",      requestorDri: "TBD",                nextAction: "Compute team scoping" },
    { id: "b-nebula",    team: "RDB-PG", name: "On-prem graph DB (NebulaGraph)",                         theme: "Cust Exp",   readiness: "watch",  problemValue: "Blocked on CAPI vs SSCP deployment strategy. Ownership unclear before any investment.", successMetric: "NebulaGraph live for ≥1 production AI use case.", effort: "TBD", earliest: "TBD",      requestorDri: "Sara M",             nextAction: "Clarify CAPI vs SSCP ownership" },
    { id: "b-clickhs",   team: "SIM",    name: "ClickHouse Cloud replacement (StarRocks)",                theme: "Efficiency",  readiness: "watch",  problemValue: "Replacement only viable if Q2/Q3 POC validates ≥40% cost saving vs ClickHouse.", successMetric: "≥40% cost saving confirmed; migration plan approved.", effort: "XL",  earliest: "Q3 2026",  requestorDri: "Danny Yuan",         nextAction: "Await Q3 POC results" },
    { id: "b-r3-disagg", team: "RDB-KV", name: "R3 disaggregation workaround (Creator 10PB risk)",       theme: "Efficiency",  readiness: "watch",  problemValue: "Creator 10PB volume exceeds local SSD limits. Full disaggregation deferred to H1 2028.", successMetric: "Workaround in place; Creator volume growth managed to H1 2028.", effort: "TBD", earliest: "Q3 2026",  requestorDri: "TBD",                nextAction: "Scoping workaround Q3" },
  ];
  ...
  const q2Asg: Array<[string, string, number]> = [
    ...
    ["Weiji Hu","ktlo",0.2],["Leon Gao","ktlo",0.2],["Leo Luo","ktlo",0.2],["Ankur Kulshrestha","ktlo",0.2],["Derek Pham","ktlo",0.2],["Julien Mo","ktlo",0.2],
    ["Will Hodges","ktlo",0.2],["Shenglin Du","ktlo",0.2],["Fred Liu","ktlo",0.2],["Gavin Wang","ktlo",0.2],["Hieu Pham","ktlo",0.2],["George Li","ktlo",0.2],["Jose Manjarrez","ktlo",0.3],["Manav Kapoor","ktlo",0.3],["Bing Yang","ktlo",0.1],
    ["Rahul Yadav","ktlo",1.0],["Ravi Gullapalli","ktlo",0.2],["Gopal Anand","ktlo",0.2],
    ["Zhixin Wen","creator",0.5],["Will Hodges","creator",0.8],
    ["Stephen Ma","frost",0.8],["Trung Dinh","frost",0.3],
    ["Meng Xu","cell",0.8],["Michael Krishnan","cell",0.8],["Shenglin Du","cell",0.8],
    ["Austen Schunk","raft",0.8],["Hieu Pham","raft",0.8],
    ["Haocheng Zuo","multitenancy",0.5],["Zhixin Wen","multitenancy",0.3],
    ["Haocheng Zuo","bandrestore",0.5],
    ["Trung Dinh","flow",0.5],["Cory Zhao","flow",0.6],["Zhengyin Qian","flow",0.8],["Nathan Zhang","flow",0.2],
    ["Shiming Song","staging",0.6],["Shiming Song","oncalltoil",0.2],["Jin Wang","staging",0.7],["Weiji Hu","staging",0.8],["Fred Liu","staging",0.8],
    ...
    ["Zijian Xie","pgint",0.5],["Bing Yang","pgint",0.9],
    ["Yingjie He","vsearch",0.9],["Shichao Jin","vsearch",0.2],
    ["Huizhi Lu","qaascreator",1.0],
    ["Ankur Kulshrestha","r3dr",0.8],
    ["Leon Gao","r3ec",0.8],
    ["Vikash Singh","pgfound",1.0],["Shichao Jin","pgfound",0.7],["Steven Wang","pgfound",0.5],
